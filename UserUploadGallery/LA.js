...
var GraphicsCategoryVO = function (id, name, graphics) {
    if (isNullOrUndefined(id)) id = 'graphics-category-' + (new Date().getTime());
    if (isNullOrUndefined(name)) name = '';
    if (isNullOrUndefined(graphics)) graphics = [];

    var self = this;
    self.id = ko.observable(id);
    self.name = ko.observable(name);
    self.graphics = ko.observable(graphics);
    ...
    self.fromObject = function (obj) {
        ...
        if (!isNullOrUndefined(obj['graphics'])) {
            self.graphics(obj['graphics']);
        }
    }
}
...
var ProductCategoryVO = function (id, name, thumbUrl, products) {
    if (isNullOrUndefined(id)) id = 'product-category-' + (new Date().getTime());
    if (isNullOrUndefined(name)) name = '';
    if (isNullOrUndefined(thumbUrl)) thumbUrl = '';
    if (isNullOrUndefined(products)) products = [];
    ...
    self.fromObject = function (obj) {
        ...
        if (!isNullOrUndefined(obj['products'])) {
...
function LAControlsModel() {
    ...
    self.selectProductCategory = function (category) {
        self.selectedProductCategoryVO().fromObject(category);
        if (!isNullOrUndefined(liveartUI.productCategories)) {
            liveartUI.productCategories.hide();
        }
        if (!isNullOrUndefined(liveartUI.productsGallery)) {
            liveartUI.productsGallery.show();
        }
    };

    self.hideProductsGallery = function () {
        if (!isNullOrUndefined(liveartUI.productCategories)) {
            liveartUI.productCategories.show();
        }
        if (!isNullOrUndefined(liveartUI.productsGallery)) {
    ...
    // selected product's category value object
    self.selectedProductCategoryVO = ko.observable(new ProductCategoryVO());

    self.productsSearchQuery = ko.observable("");
    self.currentProducts = ko.computed(function () {
        var query = this.productsSearchQuery().toLowerCase();
        var result = [];

        if (query.length > 0) {
            var categories = self.productCategories();
            var currentCategory;
        ...
        } else if (this.selectedProductCategoryVO() != null) {
            result = this.selectedProductCategoryVO().products();
        }
        return result;
    }, self);
    ...
    // selected graphics category value object
    self.selectedGraphicsCategoryVO = ko.observable(new GraphicsCategoryVO());

    // list of graphics categories
    self.graphicsCategories = ko.observableArray();

    self.selectGraphicsCategory = function (category) {
        self.selectedGraphicsCategoryVO().fromObject(category);
        if (!isNullOrUndefined(liveartUI.graphicsCategories)) {
            liveartUI.graphicsCategories.hide();
        }
        if (!isNullOrUndefined(liveartUI.graphicsGallery)) {
            liveartUI.graphicsGallery.show();
        }
    };

    self.selectGraphics = function (graphics) {
        if (graphics && graphics.id) {
            userInteract({ addGraphics: graphics.id });
            liveartUI.closeActiveTab();
        }
    }

    self.hideGraphicsGallery = function () {
        if (!isNullOrUndefined(liveartUI.graphicsCategories)) {
            liveartUI.graphicsCategories.show();
        }
        if (!isNullOrUndefined(liveartUI.graphicsGallery)) {
    ...
    self.currentGraphics = ko.computed(function () {
        var query = this.graphicsSearchQuery().toLowerCase();
        var result = [];

        if (query.length > 0) {
            var categories = self.graphicsCategories();
            var currentCategory;
            ...
            for (var i = 0; i < categories.length; i++) {
                ...
                result = result.concat(
                    ko.utils.arrayFilter(
                        ...
                        function (image) {
                            return image.name.toLowerCase().indexOf(query) > -1 || image.description.toLowerCase().indexOf(query) > -1;
                        }
                    )
                );
            }
        } else if (this.selectedGraphicsCategoryVO() != null) {
            result = this.selectedGraphicsCategoryVO().graphics();
        }
 ...
 */
    ...
    self.showUploadConditions = function(type) {
        if (type == 'url' && self.customImageUrl().length == 0)
            return;
    ...
    self.addImageByUrl2 = function(url) {
        userInteract({
                addCustomGraphics: url
            });
    ...
    self.update = function (model) {
        ...
        if (isInvalid(invalidateList, 'selectedProductCategory')) {
            self.selectProductCategory(model.selectedProductCategory);
...
function showColorCountDialog() {
    ...
    //jQuery("#liveart-color-count-popup").modal("show");
